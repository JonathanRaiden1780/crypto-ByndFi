...
import { badges, badges1, Outlinebadgesdata } from "../../../shared/data/elements/badgesdata";
...
const Badges = () => {
	...
	return (
		<Fragment>
			<Seo title={"Badge"} />
			<PageheaderDB heading="Ui Elements" active="Badge" />
			<div className="main-container container-fluid">
				<Row>
					<Col xl={6}>
						<ShowCode title="Badges" customCardClass="custom-card" customCardBodyClass="d-flex flex-wrap gap-2" code={badge1}>
							{badges.map((badge: any) => (
								<Badge key={Math.random()} className={`badge bg-${badge.color} ${badge.class} me-1`}>{badge.heading}</Badge>
							))}
						</ShowCode>
					</Col>
					<Col xl={6}>
						<ShowCode title="Pill badges" customCardClass="custom-card" customCardBodyClass="d-flex flex-wrap gap-2" code={badge2}>
							{badges.map((badge: any) => (
								<Badge key={Math.random()} className={`badge bg-${badge.color} ${badge.class} rounded-pill me-1 `}>{badge.heading}</Badge>
							))}
						</ShowCode>
					</Col>
				</Row>
					...
					<Col xl={6}>
						<ShowCode title="Light Badges" customCardClass="custom-card" customCardBodyClass="d-flex flex-wrap gap-2" code={badge3}>
							{Outlinebadgesdata.map((badge) => (
								<Badge key={Math.random()} className={`badge bg-${badge.color}-transparent ${badge.class} me-1`}>{badge.heading}</Badge>
							))}
						</ShowCode>
					</Col>
					<Col xl={6}>
						<ShowCode title="Light Pill Badges" customCardClass="custom-card" customCardBodyClass="d-flex flex-wrap gap-2" code={badge4}>
							{Outlinebadgesdata.map((badge) => (
								<Badge key={Math.random()} className={`badge bg-${badge.color}-transparent ${badge.class} rounded-pill me-1`}>{badge.heading}</Badge>
							))}
						</ShowCode>
